...
namespace ValidadorInscricaoEstadual
{
    class Program
    {
        ...
        {
            InscricaoEstadual inscricao = new InscricaoEstadual();
            Console.WriteLine( "Digite a sigla do estado" );
            inscricao.Estado = Console.ReadLine().ToUpper();
            Console.WriteLine( "Digite a inscrição estadual" );
            inscricao.Inscricao = Console.ReadLine().ToUpper();

            Console.WriteLine( "Inscrição Estadual {0} para o estado {1}", ValidarInscricaoEstadual( inscricao.Estado, inscricao.Inscricao ) ? "válido" : "inválido", inscricao.Estado );
            Console.ReadKey();
        }

        static bool ValidarInscricaoEstadual( string estado, string inscricao )
        {
            bool retorno = false;
            string strBase;
            string strBase2;
